...
{
	////////// resources /////////////////
#pragma region SHADERS AND MESHES
...
#pragma endregion
	
	{
		...
		TTN_Transform treeTrans = TTN_Transform(glm::vec3(0.f, -5.f, 1.f), glm::vec3(0.0f), glm::vec3(1.0f));
		//and attach that transform to the entity
...
void Review3Scene::Update(float deltaTime)
{
	...
	if (tBoat.GetPos().x <= 30.f) {
		...
		pBoat.SetLinearVelocity(Seek(glm::vec3(0.0f, -5.0f, 5.0f), pBoat.GetLinearVelocity(), tBoat.GetPos(), 0));
	}
...
void Review3Scene::CreateCannonball() {
	...
	//cannonballs[ballCount] = CreateEntity();
	ball = CreateEntity();//cannonballs[ballCount]; //
	cannonballs.push_back(ball);

	//Attach<TTN_Renderer>(ball);
	//auto& ballRenderer = Get<TTN_Renderer>(ball);
	...
	glm::vec3 forward = glm::mat3(glm::inverse(transCamera.GetMatrix())) * glm::vec3(0.0f, 1.0f, 0.0f);
	glm::vec3 origin = transCamera.GetPos() + forward;

	ballTrans.SetPos(glm::vec3((origin.x, origin.y +0.1f, origin.z)));
	std::cout << glm::to_string(transCamera.GetRotation()) << std::endl;
	...
	pbody.SetLinearVelocity(glm::vec3(0.f, -1.0f, 10.0f));
	AttachCopy<TTN_Physics>(ball, pbody);
	ballCount++;
}

void Review3Scene::BoatPathing(float deltaTime, entt::entity boatt, int path)
{
	auto& pBoat = Get<TTN_Physics>(boat);
	auto& tBoat = Get<TTN_Transform>(boat);
...
glm::vec3 Review3Scene::Seek(glm::vec3 target, glm::vec3 velo, glm::vec3 pos, float deltatime)
{
