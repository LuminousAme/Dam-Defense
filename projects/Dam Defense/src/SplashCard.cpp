...
void SplashCard::InitScene()
{
	...
	//main camera
	{
		...
		Get<TTN_Camera>(cam).CalcOrtho(0.0f, 1920.0f / 120.0f, 1080.0f / 120.0f, 0.0f, 0.0f, 10.0f);
	...
	//background
	{
		...
		TTN_Transform bgTrans = TTN_Transform(glm::vec3(0.0f, 0.0f, 5.0f), glm::vec3(0.0f), glm::vec3(1920.0f / 120.0f, 1080.0f / 120.0f, 1.0f));
	...
	//logo
	{
		...
		TTN_Transform logoTrans = TTN_Transform(glm::vec3(960.0f / 120.0f, 540.0f / 120.0f, 3.5f), glm::vec3(0.0f), glm::vec3(500.0f / 120.0f, 500.0f / 120.0f, 1.0f));
...
void SplashCard::Update(float deltaTime)
{
	...
	//have the logo fade in and out over the course of 6 seconds, holding for 2 seconds in the middle 
	if (totalSceneTime < 2.0f)
		Get<TTN_Renderer2D>(logo).SetColor(glm::vec4(1.0f, 1.0f, 1.0f, TTN_Interpolation::InverseLerp(0.0f, 2.0f, totalSceneTime)));
	else if (totalSceneTime >= 2.0f && totalSceneTime < 4.0f) 
		Get<TTN_Renderer2D>(logo).SetColor(glm::vec4(1.0f));
	else if (totalSceneTime >= 4.0f && totalSceneTime < 6.0f)
		Get<TTN_Renderer2D>(logo).SetColor(glm::vec4(1.0f, 1.0f, 1.0f, TTN_Interpolation::InverseLerp(6.0f, 4.0f, totalSceneTime)));
	else 
